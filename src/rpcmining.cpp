...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    if (hasPoS)
    {
        ...
        struct AMOUNT subsidy = GetProofOfStakeReward(pindexBest);
        if (subsidy.nValue > 0)
        {
             Object objSub;
             objSub.push_back(Pair(COLOR_TICKER[subsidy.nColor], (uint64_t)subsidy.nValue));
             obj.push_back(Pair("stakereward", objSub));
        }
    }
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "ponycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "ponycoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "ponycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "ponycoin is downloading blocks...");
