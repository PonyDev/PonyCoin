...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, Object& entry)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
        {
            a.push_back(CBitcoinAddress(addr, prevtxout.nColor).ToString());
        }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        Object out;
        out.push_back(Pair("value", ValueFromAmount(txout.nValue, txout.nColor)));
...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            "listunspent [minconf=1] [maxconf=9999999]  [\"address\",...] [ticker]\n"
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid ponycoin address: ")+input.get_str());
    ...
    for (adit = setAddress.begin(); adit != setAddress.end(); ++adit)
    {
        ...
        {
            throw runtime_error(strprintf("Addresse(s) do not match currency %s.",
                                                                COLOR_TICKER[nColor]));
        }
    }
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        entry.push_back(Pair("amount",ValueFromAmount(nValue, nColor)));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(Value& input, inputs)
    {
        ...
        // raw transaction does no checks ??
        // uint256 hash(txid);
        ...
        //      if (fFoundColor)
        //      {
        //            if (nColor != wtx.GetColor())
        //            {
        //                   throw runtime_error("Not all inputs are the same currency.\n");
        //            }
        //      }
        //      else
        //      {
...
Value decodescript(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "decodescript <hex string> [ticker]\n"
