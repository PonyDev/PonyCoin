...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("ponycoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  ponycoind [options]                     " + "\n" +
                  "  ponycoind [options] <command> [params]  " + _("Send command to -server or ponycoind") + "\n" +
                  "  ponycoind [options] help                " + _("List commands") + "\n" +
                  "  ponycoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "ponycoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("ponycoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("ponycoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: ponycoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: ponycoind.pid)") + "\n" +
        ...
        "  -ridethewhitepony " + _("Mint PoS bonus blocks when first nonzero of block hash is 8") + "\n" ;
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    assert ((int) PNY_COLOR_END == N_COLORS);
    ...
    // set the number of staking currencies
    {
        ...
        for (int nColor = 1; nColor < N_COLORS; ++nColor)
        {
             ...
             if (nMintColor != (int) PNY_COLOR_NONE)
             {
    ...
    else if ((nDefaultStake == PNY_COLOR_NONE) && (nNumberOfStakingCurrencies > 1))
    {
        std::string strDefaultStake = GetArg("-defaultstake", COLOR_TICKER[PNY_COLOR_NONE]);
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  ponycoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "ponycoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("ponycoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    // clear net is enabled by default
    bool fBuiltinTor = false;
    bool fExternalTor = false;

    // -onlynet indicates a specific network selection and not to use clear net unless explicit
...
/* Reserve Balances */
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of ponycoin") << "\n";
        }
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart ponycoin to complete") << "\n";
