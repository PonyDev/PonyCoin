...
CoinControlDialog::CoinControlDialog(QWidget *parent) :
    ...
    model(0),
    nColor(PNY_COLOR_NONE)
{
    ui->setupUi(this);

    ui->currencyIndicator->setText(QString(COLOR_NAME[nColor]));
...
void CoinControlDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel() && model->getAddressTableModel())
    {
        updateView();
        //updateLabelLocked();
        CoinControlDialog::updateLabels(model, this, nColor);
    }
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog, int nColor)
{
    if (!model) return;
    ...
    model->getOutputs(vCoinControl, nColor, vOutputs);
    ...
    if (nQuantity > 0)
    {
        ...
        // Fee
        int64_t nFee = vTransactionFee[nFeeColor] * (1 + (int64_t)nBytes / 1000);
        ...
        if (nPayAmount >= 0)
        {
            ...
            if (nFeeColor == nColor)
            {
                ...
                // TODO: refactor this because the same stuff is done in CWallet::CreateTransaction
                // if sub-cent change is required, the fee must be raised to at least CTransaction::nMinTxFee   
                if ((nPayFee < minTxFee) && (nChange > 0) && (nChange < CENT[nColor]/10))
                {
                    if (nChange < CENT[nColor]/10) // change < 0.01 => simply move all change to fees
                    {
                        nPayFee += nChange;
                        nChange = 0;
                    }
                    ...
                    {
                        nChange = nChange + nPayFee - CENT[nColor];
                        nPayFee = CENT[nColor];
                    }  
                }
                if (nChange == 0)
            ...
            else
            {
                ...
                nAfterFee = nAmount;
                // assume at least one more input for the fee delegate
                nBytes += 34;
            }
    ...
    int nFeeDisplayUnit = model->getOptionsModel()->getDisplayUnit(nFeeColor);
    ...
    l2->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, nAmount, nColor));        // Amount
    l3->setText(BitcoinUnits::formatWithUnit(nFeeDisplayUnit, nPayFee, nFeeColor));  // Fee
    l4->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee, nColor));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                // Bytes
    l6->setText(sPriorityLabel);                                                     // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no")));         // Low Output / Dust
    l8->setText(BitcoinUnits::formatWithUnit(nDisplayUnit, nChange, nColor));        // Change
    
    // turn labels "red"
    l5->setStyleSheet((nBytes >= 10000) ? "color:red;" : "");               // Bytes >= 10000
    l6->setStyleSheet((dPriority <= 576000) ? "color:red;" : "");         // Priority < "medium"
    l7->setStyleSheet((fLowOutput) ? "color:red;" : "");                    // Low Output = "yes"
    l8->setStyleSheet((nChange > 0 && nChange < CENT[nColor]) ? "color:red;" : ""); // Change < 0.01BTC
        
    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, CENT[nColor], nColor)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, CENT[nColor], nColor)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, MIN_TXOUT_AMOUNT[nColor], nColor)).arg(BitcoinUnits::formatWithUnit(nDisplayUnit, MIN_TX_FEE[nFeeColor], nFeeColor)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, CENT[nFeeColor], nFeeColor)).arg(BitcoinUnits::formatWithUnit(nDisplayUnit, MIN_TX_FEE[nFeeColor], nFeeColor)));
...
void CoinControlDialog::updateView()
{
    ...
    {
        nDisplayUnit = model->getOptionsModel()->getDisplayUnit(nColor);
    }
        
    map<QString, vector<COutput> > mapCoins;
    model->listCoins(nColor, mapCoins);
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                int nAddrColor = out.tx->vout[out.i].nColor;
                sAddress = CBitcoinAddress(outputAddress, nAddrColor).ToString().c_str();
                
                // If listMode or change => show bitcoin address.
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BitcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue, nColor));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BitcoinUnits::format(nDisplayUnit, nSum, nColor));
...
int CoinControlDialog::getColor()
{
    return nColor;
}

bool CoinControlDialog::setColor(int nColorIn)
{
