...
{
    QString ret = QString("ponycoin:%1").arg(address);
    int paramCount = 0;

    CBitcoinAddress addr(std::string(address.toUtf8().constData()));
    ui->outUri->clear();
    ...
    if (ui->chkReqPayment->isChecked())
    {
        if (ui->lnReqAmount->validate())
        {
            ...
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BTC, ui->lnReqAmount->value(), addr.nColor));
            paramCount++;
        }
        else
        {
...
void QRCodeDialog::updateDisplayUnit()
{
    if (model)
    {
        ...
        ui->lnReqAmount->setDisplayUnit(model->getDisplayUnit(addr.nColor));
    }
}
