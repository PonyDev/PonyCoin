...
{
    int unitPonyCoin = model->getOptionsModel()->getDisplayUnitPonyCoin();
    ...
    ui->labelBalancePonyCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPonyCoin, mapSpendable[PNY_COLOR_PONY], PNY_COLOR_PONY));
    ui->labelStakePonyCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPonyCoin, mapCurrentStake[PNY_COLOR_PONY], PNY_COLOR_PONY));
    ui->labelUnconfirmedPonyCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPonyCoin, mapCurrentUnconfirmedBalance[PNY_COLOR_PONY], PNY_COLOR_PONY));
    ui->labelImmaturePonyCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPonyCoin, mapCurrentImmatureBalance[PNY_COLOR_PONY], PNY_COLOR_PONY));
    ui->labelTotalPonyCoin->setText(BitcoinUnits::formatWithUnitLocalized(
             unitPonyCoin, mapSpendable[PNY_COLOR_PONY] +
                           mapCurrentStake[PNY_COLOR_PONY] +
                           mapCurrentUnconfirmedBalance[PNY_COLOR_PONY], PNY_COLOR_PONY));

    // only show unconfirmed, stake, immature balances non-zero, so as not to complicate things
    bool showUnconfirmedPonyCoin = mapCurrentUnconfirmedBalance[PNY_COLOR_PONY] != 0;
    ui->labelUnconfirmedPonyCoin->setVisible(showUnconfirmedPonyCoin);
    ui->lblUniUn->setVisible(showUnconfirmedPonyCoin);
    bool showStakePonyCoin = mapCurrentStake[PNY_COLOR_PONY] != 0;
    ui->labelStakePonyCoin->setVisible(showStakePonyCoin);
    ui->lblUniStk->setVisible(showStakePonyCoin);
    bool showImmaturePonyCoin = mapCurrentImmatureBalance[PNY_COLOR_PONY] != 0;
    ui->labelImmaturePonyCoin->setVisible(showImmaturePonyCoin);
    ui->lblUniImm->setVisible(showImmaturePonyCoin);
...
void OverviewPage::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPonyCoin(int)),
