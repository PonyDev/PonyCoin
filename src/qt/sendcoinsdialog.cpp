    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(
                      tr("Enter a PonyCoin address (e.g. brj6Q4FUHXjAzCRb6SB3pXPfuva5xrfjgHY)"));
    ...
    ui->comboCurrency->addItem(COLOR_NAME[PNY_COLOR_PONY],
                               (int) PNY_COLOR_PONY);
...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        setBalance(mapBalance, mapStake, mapUnconfirmedBalance, mapImmatureBalance);
        connect(model, SIGNAL(balanceChanged(std::map<int, qint64>, std::map<int, qint64>,
                                             std::map<int, qint64>, std::map<int, qint64>)), this,
                       SLOT(setBalance(std::map<int, qint64>, std::map<int, qint64>,
                                       std::map<int, qint64>, std::map<int, qint64>)));
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPonyCoin(int)), this, SLOT(updateDisplayUnit()));
        connect(model->getOptionsModel(), SIGNAL(defaultColorChanged(int)), this, SLOT(currencyChanged(int)));

        // Coin Control
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedPonyCoin(int)), this, SLOT(coinControlUpdateLabels()));
        connect(model->getOptionsModel(), SIGNAL(coinControlFeaturesChanged(bool)), this, SLOT(coinControlFeatureChanged(bool)));
        connect(model->getOptionsModel(), SIGNAL(transactionFeeChangedPonyCoin(qint64)), this, SLOT(coinControlUpdateLabels()));
...
void SendCoinsDialog::setChangeAddressPlaceholderText(int nColor)
{
    ...
    case PNY_COLOR_PONY:
        ...
        /* Do not move this to the XML file, Qt before 4.7 will choke on it */
        ui->lineEditCoinControlChange->setPlaceholderText(
                    tr("Enter a PonyCoin address (e.g. bxMpqqmeKUFfFeVYJn3iHK9UvjdhApUrcNq)"));
#endif
        break;
    default:
