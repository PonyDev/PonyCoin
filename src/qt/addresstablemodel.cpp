    ...
    void refreshAddressTable()
    {
        cachedAddressTable.clear();
        {
            ...
            for (it = wallet->stealthAddresses.begin(); it != wallet->stealthAddresses.end(); ++it)
            {
                ...
                cachedAddressTable.append(AddressTableEntry(fMine ?
                            AddressTableEntry::Receiving : AddressTableEntry::Sending,
...
{
    columns << tr("Label") << tr("Currency") << tr("Address");
...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Ticker:
            return tickerForAddress(rec->address);
        case Address:
            return rec->address;
        case Color:
            return colorForAddress(rec->address);
        }
    }
    ...
    else if (role == TypeColorRole)
    {
        // TODO: cache color?
        int nColor = colorForAddress(rec->address);
        switch (rec->type)
        {
        case AddressTableEntry::Sending:
            return QString("%1-%2").arg(Send).arg(nColor);
        case AddressTableEntry::Receiving:
            return QString("%1-%2").arg(Receive).arg(nColor);
        default: break;
        }
    }
    return QVariant();
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                CBitcoinAddress(addr) = CBitcoinAddress(strTemp);
                wallet->SetAddressBookName(addr.Get(), addr.nColor,
        ...
        case Address:
            ...
            else if(walletModel->validateAddress(value.toString()) == PNY_COLOR_NONE)
            {
                editStatus = INVALID_ADDRESS;
                return false;
            }
            ...
            {
                {
                    ...
                    CBitcoinAddress addr(value.toString().toStdString());
                    // Add new entry with new address
                    wallet->SetAddressBookName(addr.Get(), addr.nColor, rec->label.toStdString());
                }
            }
            break;
        }
...
QString AddressTableModel::addRow(const QString &type, int nColor, const QString &label,
                                          const QString &address, int addressType)
{
    ...
    if (type == Send)
    {
            ...
            {
                if (walletModel->validateAddress(address) == PNY_COLOR_NONE)
                {
                    editStatus = INVALID_ADDRESS;
                    return QString();
                };
                // Check for duplicate addresses
                {
                    ...
                    wallet->SetAddressBookName(addr.Get(), addr.nColor, strLabel);
                }
            }
    }
    else if (type == Receive)
    {
        ...
        if (addressType == AT_Stealth)
        {
            ...
            std::string sError;
            if (!wallet->NewStealthAddress(nColor, sError, strLabel, newStealthAddr)
        ...
        } else
        {
            ...
            strAddress = CBitcoinAddress(newKey.GetID(), nColor).ToString();
            {
                LOCK(wallet->cs_wallet);
                CBitcoinAddress addr(strAddress);
                wallet->SetAddressBookName(addr.Get(), addr.nColor, strLabel);
...
int AddressTableModel::colorForAddress(const QString &address) const
{
    LOCK(wallet->cs_wallet);
    //CBitcoinAddress address_parsed(address.toStdString());
    std::string sAddr = address.toStdString();
    ...
    if (sAddr.length() > 75)
    {
        ...
        if (!sxAddr.SetEncoded(sAddr))
        {
            printf("Could not encode stealth address %s\n", address.toStdString().c_str());
            return PNY_COLOR_NONE;
        }
    ...
    {
        CBitcoinAddress address_parsed(sAddr);
        return address_parsed.nColor;
    }
    return PNY_COLOR_NONE;
}

QString AddressTableModel::tickerForAddress(const QString &address) const
{
    int nColor = colorForAddress(address);
    if (!CheckColor(nColor))
    {
        if (fDebug)
        {
            printf("Address %s has invalid currency %d\n", address.toStdString().c_str(), nColor);
        }
        nColor = PNY_COLOR_NONE;
    }
    return QString(COLOR_TICKER[nColor]);
}
