...
int64_t AmountFromValue(const Value& value, int nColor, bool checkMoneyRange)
{
    // None currency has no value ever.
    if (nColor == PNY_COLOR_NONE)
    {
        return 0;
    }
    if (nColor < 0 || nColor > N_COLORS)
    {
        throw JSONRPCError(RPC_TYPE_ERROR, strprintf("Invalid currency: %d", nColor));
    }
    double dAmount = value.get_real();
    if (dAmount <= 0.0 || (checkMoneyRange && dAmount > MAX_MONEY[nColor]))
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    int64_t nAmount = roundint64(dAmount * COIN[nColor]);
    if (checkMoneyRange && !MoneyRange(nAmount, nColor))
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    return nAmount;
}

Value ValueFromAmount(int64_t amount, int nColor)
{
    // None currency has no value ever.
    if (nColor == PNY_COLOR_NONE)
    {
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop ponycoin server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "ponycoin server stopping";
}
...
{ //  name                         function                 safemd  unlocked
  //  ------------------------     -----------------------  ------  --------
    ...
    // TODO: need to dynamically filter help for this if no staking currencies
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: ponycoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use ponycoind";
...
#ifdef IMPORT_WALLET
    if (strMethod == "encodebase58"                 && n > 1) ConvertTo<bool>(params[1]);
