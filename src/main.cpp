...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 18);
...
unsigned int nTargetSpacingPoS = 120; // 2 min
...
int nCoinbaseMaturity = 180; // 180 blocks (6 hr)
...
const string strMessageMagic = "PonyCoin Signed Message:\n";

// Settings

// nDefaultCurrency and nDefaultStake are for sideways compatibility.
int nDefaultCurrency = (int) PNY_COLOR_NONE;
...
int nDefaultStake = (int) PNY_COLOR_NONE;
...
// yeah they aren't vectors, but too late now
std::vector<int64_t> vTransactionFee(MIN_TX_FEE, MIN_TX_FEE + N_COLORS);
std::vector<int64_t> vReserveBalance(N_COLORS, 0);
std::vector<int64_t> vMinimumInputValue(MIN_INPUT_VALUE,
...
bool CTransaction::CheckTransaction() const
{
    ...
    std::vector<int64_t> vValueOut(N_COLORS, 0);
    for (unsigned int i = 0; i < vout.size(); i++)
    {
...
int64_t CTransaction::GetOpRetFee() const {
    ...
    std::sort(vOpRetFees.begin(), vOpRetFees.end(), std::greater<int>());
...
struct AMOUNT GetPoWSubsidy(int nHeight)
{
    ...
    static const int PENULTIMATE_POW_BLOCK = LAST_POW_BLOCK - 1;

    struct AMOUNT stSubsidy;
    stSubsidy.nValue = 0;
    // the only PoW reward currency is PONY
    stSubsidy.nColor = (int) PNY_COLOR_PONY;

    int day = nHeight / 720 ; // 120 s blocks

    if (nHeight <= PENULTIMATE_POW_BLOCK)
    {
        stSubsidy.nValue = COIN[stSubsidy.nColor] * GetDayReward(day);
    }
...
struct AMOUNT GetProofOfWorkReward(int nHeight)
{
    ...
    {
        printf("GetProofOfWorkReward() : create=%s nSubsidy=%" PRId64 " nColor=%d\n",
                               FormatMoney(stSubsidy.nValue, stSubsidy.nColor).c_str(),
                                                         stSubsidy.nValue, stSubsidy.nColor);
    }
    return stSubsidy;
}
...
{
    int nMintColor = (int) PNY_COLOR_PONY;
    int nStakeColor = (int) PNY_COLOR_PONY;
    int64_t nCent = CENT[PNY_COLOR_PONY];
...
unsigned int PonyCoinGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    unsigned int count;
    std::vector<const CBlockIndex *>vpIndex(BLOCKSPAN);
        ...
        if(CountBlocks <= PastBlocksMin)
        {
            ...
            else
            {
                PastDifficultyAverage = ((PastDifficultyAveragePrev * CountBlocks) +
                               (CBigNum().SetCompact(BlockReading->nBits))) / (CountBlocks + 1);
            }
...
{
    return PonyCoinGravityWave(pindexLast, fProofOfStake);
}
...
void CTransaction::FillValuesIn(const MapPrevTx& inputs, std::map<int, int64_t> &mapValuesIn) const
{
    mapValuesIn.clear();
    if (IsCoinBase())
    {
    ...
    for (unsigned int i = 0; i < vin.size(); ++i)
    {
        ...
        if (txOut.nValue != 0)
        {
            std::map<int, int64_t>::iterator it = mapValuesIn.find(nColor);
            if (it == mapValuesIn.end())
            {
                mapValuesIn.insert(std::pair<int, int64_t>(txOut.nColor, txOut.nValue));
            }
            else
            {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // TODO: these may be ok for hundreds of currencies, but will be a waste for 100k, etc.
    // fees are in - out for non-coinbase transactions
    std::vector<int64_t>vFees(N_COLORS, 0);
    std::vector<int64_t>vValueIn(N_COLORS, 0);
    std::vector<int64_t>vValueOut(N_COLORS, 0);
    std::vector<int64_t>vScavengedFees(N_COLORS, 0);

    int64_t nStakeReward = 0;
    int nStakeColor = (int) PNY_COLOR_NONE;

    // *claimed* subsidy from GetProofOfStakeReward()
    int64_t nMintValue = 0;
    int nMintColor = (int) PNY_COLOR_NONE;
    ...
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
        ...
        else
        {
            ...
            std::map<int, int64_t> mapValuesIn;
            tx.FillValuesIn(mapInputs, mapValuesIn);
            std::map<int, int64_t>::iterator iit;
            ...
            if (tx.IsCoinStake())
            {
                ...
                nStakeReward = mapValuesOut[nColor] - mapValuesIn[nColor];
                if (nStakeReward > 0)
                {
    ...
    if (IsProofOfWork())
    {
        ...
        //      color  amount
        std::map<int, int64_t> mapCoinBaseValuesOut;
        // fFees=true looks at vout[1] and later (vout[0] already checked)
        txCoinBase.FillValuesOut(mapCoinBaseValuesOut, true);
        std::map<int, int64_t>::iterator oit;
        for (oit = mapCoinBaseValuesOut.begin(); oit != mapCoinBaseValuesOut.end(); ++oit)
        {
            // reject any coinbase reward currency in second and later transactions
            // this makes accounting simpler elswhere (may change this)
            // for PoS, stake is in vtx[1]
            if (oit->first == reward.nColor)
            {
                 return DoS(100, error("ConnectBlock() : reward currency in non-coinbase output\n"));
            }
            else if (oit->second > vFees[oit->first])   // already checked coinbase currency
            {
                 return DoS(100, error("ConnectBlock() : fees exceeded for currency %s"
                                       "(actual=%" PRId64 " vs calc=%" PRId64 ")",
                                  COLOR_TICKER[oit->first], oit->second, vFees[oit->first]));
            }
        }

        // not a PoS block, set the pindex stake color to none
        pindex->nStakeColor = (int) PNY_COLOR_NONE;
    ...
    if (IsProofOfStake())
    {
        ...
        // ponycoin: reward based nHeight and color
        struct AMOUNT reward = GetProofOfStakeReward(pindex->pprev);

        if ((PonyCheck(GetHash().ToString())) && (pindex->nHeight <= LAST_WHITEPONY_BLOCK))
        {
            reward.nValue *= 100;   // 100x bonus block
        }

        // ponycoin: reward is earned in the mint transaction, not the stake transaction
...
// A coin-day is really a BASE_COIN-day (see globalparams.h).
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    if (!ComputeNextStakeModifier(pindexNew->pprev, bnStakeModifier, fGeneratedStakeModifier))
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
        return DoS(50, error("CheckBlock() : coinbase timestamp: %" PRId64 " + 15 sec, "
                             "is too early for block: %" PRId64,
                                (int64_t)vtx[0].nTime, (int64_t)GetBlockTime()));

    if (IsProofOfStake())
    {
        // protocol change for multicurrencies: PoS mint is put into coinbase
        // next 2 comments kept for record
        ...
        if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64_t)vtx[0].nTime))
            return DoS(50, error("CheckBlock() : coinmint timestamp violation nTimeBlock=%" PRId64 " nTimeTx=%u", GetBlockTime(), vtx[1].nTime));
...
bool CBlock::AcceptBlock()
{
    ...
    // PonyCoin: PoS does not start until PoW is finished
    ...
    {
        printf("AcceptBlock()): Proof calculated: %d,   Proof received in block: %d\n",
                      GetNextTargetRequired(pindexPrev, IsProofOfStake()), nBits);
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pblock->IsProofOfStake() && (pblock->nNonce != 0))
    {
        ...
        return error("ProcessBlock() : PoS block has non-zero nonce: %s", hash.ToString().c_str());
    }
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees[])
{
    ...
    CKey key;
    // mint and stake could be 2 different colors, so require 2 different transactions
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
                                            ...
                                            nFees, vtx[0], txCoinStake, key))
        {
            if (txCoinStake.nTime >= pindexBest->GetPastTimeLimit()+1)
            {
                ...
                // try for bonus based on 1/16 chances
                if ((pindexPrev->nHeight < LAST_WHITEPONY_BLOCK) &&
                                  (GetBoolArg("-ridethewhitepony", false)))
                {
                    int64_t nRegular = vtx[0].vout[0].nValue;
                    int64_t nBonus = nRegular * 100;
                    vtx[0].vout[0].nValue = nBonus;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PonyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        LAST_POW_BLOCK = 120;
        FIRST_POS_BLOCK = 121;
        LAST_WHITEPONY_BLOCK = 2225;

        nTargetSpacingPoS = 10; // 10 sec
        nTargetSpacingPoW = 10; // 10 sec
        ...
        nModifierInterval = 30; // 30 s (time to elapse before new modifier is computed)
        // nLaunchTime = 1415868300;
        nStakeMaxAge = 60 * 20; // 20 minutes
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "G.O.P. Relents to Pressure, Delaying 4 Cabinet Hearings";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1484119341;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.strTxComment = "";
        txNew.vin[0].scriptSig = CScript() << 0 << CBigNum(nLaunchTime) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].SetEmpty();
        txNew.SetColor((int) PNY_COLOR_NONE);
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1484119341;
        ...
        assert(block.hashMerkleRoot ==
          uint256("0x98e3c55cc9ddd5e8475eada25da01517de54248d9f58d25a7f736e9344658f60"));
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  coinbase %9s %s  tx %"PRIszu"",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
            ...
            FormatMoney(pindex->vCoinbase[pindex->nCoinbaseColor], pindex->nCoinbaseColor).c_str(),
            COLOR_TICKER[pindex->nCoinbaseColor],
            block.vtx.size());
...
unsigned char pchMessageStart[4] = { 0xdf, 0xbb, 0xfd, 0xff };
