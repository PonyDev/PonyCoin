...
enum
{
    ...
    // PonyCoin: added to MANDATORY_SCRIPT_VERIFY_FLAGS from BLK V3 fork
...
class GetAddressColorVisitor : public boost::static_visitor<int>
{
public:
    int operator()(const CNoDestination &dest) const { return PNY_COLOR_NONE; }
    int operator()(const CKeyID &keyID) const { return keyID.nColor; }
    int operator()(const CScriptID &scriptID) const { return scriptID.nColor; }
