...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t pFees[])
{
    ...
    // fees dumped for PoS, but kept for PoW
    ...
    // Collect memory pool transactions into the block
    {
                                                          ...
                                                          mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (tx.IsCoinBase() || tx.IsCoinStake() || !tx.IsFinal() || !CheckColor(nColor))
            {
                continue;
            }
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin)
            {
                ...
                dPriority += PRIORITY_MULTIPLIER[nInputColor] * (double)nValueIn * nConf;
            }
        ...
        while (!vecPriority.empty())
        {
            ...
            for (int i = 1; i < N_COLORS; ++i)
            {
                ...
                // can't claim fees that aren't scavengable
                if (SCAVENGABLE[i])
                {
                    nFees[i] += txfee;
                }
        ...
        for (int j = 1; j < N_COLORS; ++j)
        {
            if (SCAVENGABLE[j])
            {
                int64_t nTotalMintPrev = (pindexPrev ? pindexPrev->vTotalMint[j] : 0);
                int64_t nMoneySupplyPrev = (pindexPrev ? pindexPrev->vMoneySupply[j] : 0);
        ...
        if (!fProofOfStake)
        {
            ...
            for (int i = 1; i < N_COLORS; ++i)
            {
                if ((nFees[i] > 0) && (i != reward.nColor))
                {
                    ...
                    CTxOut feeOut(nFees[i], i, pblock->vtx[0].vout[0].scriptPubKey);
...
bool CheckStake(CBlock* pblock, CWallet& wallet)
{
    ...
    if (!CheckProofOfStake(mapBlockIndex[pblock->hashPrevBlock],
                               pblock->vtx[1], pblock->nBits, proofHash, hashTarget))
